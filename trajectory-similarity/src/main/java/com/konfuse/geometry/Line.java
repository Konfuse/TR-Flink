...
public class Line extends DataObject implements Serializable {
    ...
    public double calDistance(Point point) {
        ...
        if (r <= 0){
            return Math.sqrt((point.getX() - x1) * (point.getX() - x1) + (point.getY() - y1) * (point.getY() - y1));

        }
        else if (cross >= distanceSquared){
            return Math.sqrt((point.getX() - x2) * (point.getX() - x2) + (point.getY() - y2) * (point.getY() - y2));
