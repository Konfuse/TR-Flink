...
**在这篇论文中，作者通过经验和一定的数学推导最后得到的观测概率矩阵是$\mu$ = 0 和 $\sigma$ = 4.07的高斯分布。$\sigma$使用绝对中位差进行鲁棒估计。状态转移矩阵是用指数函数来拟合前后两个相邻GPS观测点的距离与两个候选点距离之差的绝对值。在初始时刻使用观测概率矩阵。**

**在这篇论文中提出，可以使用滑动窗口的模式来处理实时数据的路网匹配（批处理模式）**

-----

12月5号 to be continued

-----

Map-matching for low-sampling-rate GPS trajectories[GIS 2009]：这篇论文提出了ST-Matching方法，这感觉这个方法实现简单,实用性也比较强。这个方法考虑了道路网络的空间几何和拓扑结构。其状态的转移由空间分析数和时间分析函数计算而成，考虑了轨迹中前后点的联系和影响。该算法本质上是一个全局匹配算法，但可以对轨迹进行切分，对每段轨迹进行并行计算。

 
DeepMM Deep Learning Based Map Matching with Data Augmentation[SIGSPATIALGIS 2019]:这篇论文使用深度学习的方法，该方法首先利用100m*100m的网格将地图进行切分，并把每个网格ID转换成one-hot向量。利用embedding方法将轨迹中的点或线段进行转换，最后使用自己设计的注意力机制的seq2seq模型来对轨迹数据进行训练和预测。由于深度学习需要大量的轨迹数据，这篇1/4的篇幅在介绍如何生成仿真轨迹数据。

 
Effective map-matching on the most simplified road network[SIGSPATIAL 2012]:这篇文章提出了一种Passby的路网匹配算法。主要针对问题是：由于路网数据占用很大的空间，如何在简化了路网信息的情况下进行路网匹配。这篇文章将路网信息中的每条路简化为（id，start，end）的集合。Passby考虑了轨迹中的前后点的关联，对每个点依次进行了匹配。该方法可以进行并行化计算，每个线程负责一部分地图数据的匹配。
